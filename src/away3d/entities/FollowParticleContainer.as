	...
	public class FollowParticleContainer extends ObjectContainer3D
	{
		...
		override public function get sceneTransform():Matrix3D
		{
			if (_sceneTransformDirty)
			{
				...
				var rawData:Vector.<Number> = Matrix3DUtils.RAW_DATA_CONTAINER;// _identityTransform.rawData;
				_identityTransform.copyRawDataTo(rawData);
				rawData[0] = comps[2].x;
				rawData[5] = comps[2].y;
...
class TargetObject3D extends ObjectContainer3D
{
	...
	private function validateTransform():void
	{
		if (_sceneTransformDirty)
		{
			...
			var comps:Vector.<Vector3D> = Matrix3DUtils.decompose(_helpTransform);//_helpTransform.decompose();
			this.specificPos.setTo(comps[0].x, comps[0].y, comps[0].z);// = comps[0];
